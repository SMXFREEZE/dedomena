import { Connector, ConnectorField, Category, CategoryId, OAuthConfig, ManagedOAuthOption } from './types';
...
// ── Managed OAuth augmentation ─────────────────────────────────────────────────
// Maps connector IDs → the one-click OAuth options offered to users.
// Option 1 (OAuth) is always shown first in the form; Option 2 (manual) is secondary.

const G = (scopes: string[]): ManagedOAuthOption => ({
  provider: 'google', label: 'Sign in with Google', iconSlug: 'google', color: '#4285f4', scopes,
});
const MS = (scopes: string[], label = 'Sign in with Microsoft'): ManagedOAuthOption => ({
  provider: 'microsoft', label, iconSlug: 'microsoftazure', color: '#0078d4', scopes: [...scopes, 'offline_access'],
});

const MANAGED: Record<string, ManagedOAuthOption[]> = {
  // ── Google ──────────────────────────────────────────────────────────────────
  gmail:                  [G(['https://www.googleapis.com/auth/gmail.readonly'])],
  'google-drive':         [G(['https://www.googleapis.com/auth/drive.readonly'])],
  'google-sheets':        [G(['https://www.googleapis.com/auth/spreadsheets.readonly'])],
  'google-docs':          [G(['https://www.googleapis.com/auth/documents.readonly'])],
  'google-slides':        [G(['https://www.googleapis.com/auth/presentations.readonly'])],
  'google-calendar':      [G(['https://www.googleapis.com/auth/calendar.readonly'])],
  'google-analytics':     [G(['https://www.googleapis.com/auth/analytics.readonly'])],
  'google-search-console':[G(['https://www.googleapis.com/auth/webmasters.readonly'])],
  'youtube-analytics':    [G(['https://www.googleapis.com/auth/youtube.readonly', 'https://www.googleapis.com/auth/yt-analytics.readonly'])],
  bigquery:               [G(['https://www.googleapis.com/auth/bigquery.readonly']), /* alt: service account below */],
  gcs:                    [G(['https://www.googleapis.com/auth/devstorage.read_only'])],
  'google-ads':           [G(['https://www.googleapis.com/auth/adwords'])],
  firebase:               [G(['https://www.googleapis.com/auth/firebase.readonly', 'https://www.googleapis.com/auth/cloud-platform.read-only'])],

  // ── Microsoft ────────────────────────────────────────────────────────────────
  outlook:               [MS(['Mail.Read'], 'Sign in with Microsoft')],
  'outlook-calendar':    [MS(['Calendars.Read'])],
  onedrive:              [MS(['Files.Read'])],
  sharepoint:            [MS(['Sites.Read.All'])],
  'microsoft-excel':     [MS(['Files.Read', 'Sites.Read.All'])],
  'microsoft-onenote':   [MS(['Notes.Read'])],
  'microsoft-teams':     [MS(['ChannelMessage.Read.All', 'Channel.ReadBasic.All', 'Team.ReadBasic.All'])],
  'dynamics-365':        [MS(['https://org.crm.dynamics.com/user_impersonation'])],
  synapse:               [MS(['https://database.windows.net//.default'])],
  'azure-devops':        [MS(['499b84ac-1321-427f-aa17-267ca6975798/.default'], 'Sign in with Microsoft')],

  // ── GitHub ────────────────────────────────────────────────────────────────────
  github: [{
    provider: 'github', label: 'Sign in with GitHub', iconSlug: 'github', color: '#ffffff',
    scopes: ['repo', 'read:org', 'read:user'],
  }],
  gitlab: [{
    provider: 'gitlab', label: 'Sign in with GitLab', iconSlug: 'gitlab', color: '#fc6d26',
    scopes: ['read_api', 'read_repository'],
  }],

  // ── Slack ────────────────────────────────────────────────────────────────────
  slack: [{
    provider: 'slack', label: 'Sign in with Slack', iconSlug: 'slack', color: '#4a154b',
    scopes: ['channels:history', 'channels:read', 'users:read', 'files:read'],
  }],

  // ── Notion ────────────────────────────────────────────────────────────────────
  notion: [{
    provider: 'notion', label: 'Connect Notion', iconSlug: 'notion', color: '#ffffff',
    scopes: [],   // Notion OAuth has no scope params; uses workspace access
  }],

  // ── Atlassian ─────────────────────────────────────────────────────────────────
  jira: [{
    provider: 'atlassian', label: 'Sign in with Atlassian', iconSlug: 'jira', color: '#0052cc',
    scopes: ['read:jira-work', 'read:jira-user', 'offline_access'],
  }],
  confluence: [{
    provider: 'atlassian', label: 'Sign in with Atlassian', iconSlug: 'confluence', color: '#0052cc',
    scopes: ['read:confluence-space.summary', 'read:confluence-content.all', 'offline_access'],
  }],

  // ── Salesforce ────────────────────────────────────────────────────────────────
  salesforce: [{
    provider: 'salesforce', label: 'Sign in with Salesforce', iconSlug: 'salesforce', color: '#00a1e0',
    scopes: ['api', 'refresh_token'],
  }],

  // ── HubSpot ───────────────────────────────────────────────────────────────────
  hubspot: [{
    provider: 'hubspot', label: 'Connect HubSpot', iconSlug: 'hubspot', color: '#ff7a59',
    scopes: ['crm.objects.contacts.read', 'crm.objects.deals.read', 'crm.objects.companies.read'],
  }],

  // ── Databricks ────────────────────────────────────────────────────────────────
  databricks: [{
    provider: 'databricks', label: 'Sign in with Databricks', iconSlug: 'databricks', color: '#ff3621',
    scopes: ['sql', 'clusters', 'jobs'],
  }],

  // ── Dropbox ───────────────────────────────────────────────────────────────────
  dropbox: [{
    provider: 'dropbox', label: 'Connect Dropbox', iconSlug: 'dropbox', color: '#0061ff',
    scopes: ['files.metadata.read', 'files.content.read'],
  }],

  // ── Box ───────────────────────────────────────────────────────────────────────
  box: [{
    provider: 'box', label: 'Connect Box', iconSlug: 'box', color: '#0061d5',
    scopes: ['root_readwrite'],
  }],

  // ── Airtable ──────────────────────────────────────────────────────────────────
  airtable: [{
    provider: 'airtable', label: 'Connect Airtable', iconSlug: 'airtable', color: '#18bfff',
    scopes: ['data.records:read', 'schema.bases:read'],
  }],

  // ── Linear ────────────────────────────────────────────────────────────────────
  linear: [{
    provider: 'linear', label: 'Connect Linear', iconSlug: 'linear', color: '#5e6ad2',
    scopes: ['read'],
  }],

  // ── Figma ─────────────────────────────────────────────────────────────────────
  figma: [{
    provider: 'figma', label: 'Connect Figma', iconSlug: 'figma', color: '#f24e1e',
    scopes: ['file_read'],
  }],

  // ── Monday ────────────────────────────────────────────────────────────────────
  monday: [{
    provider: 'monday', label: 'Connect Monday.com', iconSlug: 'mondaydotcom', color: '#ff3d57',
    scopes: ['boards:read', 'workspaces:read'],
  }],

  // ── Asana ─────────────────────────────────────────────────────────────────────
  asana: [{
    provider: 'asana', label: 'Connect Asana', iconSlug: 'asana', color: '#f06a6a',
    scopes: ['default'],
  }],

  // ── Shopify ───────────────────────────────────────────────────────────────────
  shopify: [{
    provider: 'shopify', label: 'Connect Shopify Store', iconSlug: 'shopify', color: '#96bf48',
    scopes: ['read_orders', 'read_products', 'read_customers'],
    requiresField: 'shop',   // user must enter their shop domain first
  }],

  // ── Stripe ────────────────────────────────────────────────────────────────────
  stripe: [{
    provider: 'stripe', label: 'Connect Stripe', iconSlug: 'stripe', color: '#6772e5',
    scopes: ['read_only'],
  }],

  // ── Twitter/X ─────────────────────────────────────────────────────────────────
  twitter: [{
    provider: 'twitter', label: 'Sign in with X', iconSlug: 'x', color: '#ffffff',
    scopes: ['tweet.read', 'users.read', 'bookmark.read', 'list.read'],
  }],

  // ── Discord ───────────────────────────────────────────────────────────────────
  discord: [{
    provider: 'discord', label: 'Connect Discord', iconSlug: 'discord', color: '#5865f2',
    scopes: ['guilds', 'guilds.members.read'],
  }],

  // ── Zoom ──────────────────────────────────────────────────────────────────────
  zoom: [{
    provider: 'zoom', label: 'Connect Zoom', iconSlug: 'zoom', color: '#2d8cff',
    scopes: ['meeting:read', 'recording:read', 'user:read'],
  }],

  // ── Intercom ──────────────────────────────────────────────────────────────────
  intercom: [{
    provider: 'intercom', label: 'Connect Intercom', iconSlug: 'intercom', color: '#286efa',
    scopes: [],   // Intercom uses workspace-level access grant
  }],

  // ── Typeform ──────────────────────────────────────────────────────────────────
  typeform: [{
    provider: 'typeform', label: 'Connect Typeform', iconSlug: 'typeform', color: '#262627',
    scopes: ['forms:read', 'responses:read', 'accounts:read'],
  }],

  // ── Webflow ───────────────────────────────────────────────────────────────────
  webflow: [{
    provider: 'webflow', label: 'Connect Webflow', iconSlug: 'webflow', color: '#4353ff',
    scopes: ['cms:read', 'sites:read'],
  }],

  // ── Zendesk ───────────────────────────────────────────────────────────────────
  zendesk: [{
    provider: 'zendesk', label: 'Sign in with Zendesk', iconSlug: 'zendesk', color: '#03363d',
    scopes: ['read'],
    requiresField: 'subdomain',
  }],

  // ── Plaid (handled via Plaid Link, not standard redirect) ────────────────────
  plaid: [{
    provider: 'plaid', label: 'Connect Bank with Plaid', iconSlug: 'plaid', color: '#0b3c72',
    scopes: [],
  }],

  // ── LinkedIn ──────────────────────────────────────────────────────────────────
  linkedin: [{
    provider: 'linkedin', label: 'Sign in with LinkedIn', iconSlug: 'linkedin', color: '#0a66c2',
    scopes: ['r_liteprofile', 'r_emailaddress', 'r_organization_social'],
  }],

  // ── Spotify ───────────────────────────────────────────────────────────────────
  spotify: [{
    provider: 'spotify', label: 'Connect Spotify', iconSlug: 'spotify', color: '#1db954',
    scopes: ['user-read-recently-played', 'user-top-read', 'playlist-read-private', 'user-library-read'],
  }],

  // ── QuickBooks ────────────────────────────────────────────────────────────────
  quickbooks: [{
    provider: 'quickbooks', label: 'Connect QuickBooks', iconSlug: 'intuit', color: '#2ca01c',
    scopes: ['com.intuit.quickbooks.accounting'],
  }],

  // ── Xero ──────────────────────────────────────────────────────────────────────
  xero: [{
    provider: 'xero', label: 'Connect Xero', iconSlug: 'xero', color: '#13b5ea',
    scopes: ['accounting.transactions.read', 'accounting.contacts.read', 'offline_access'],
  }],
};

// Augment CONNECTORS with managedOAuth options
CONNECTORS.forEach(c => {
  if (MANAGED[c.id]) c.managedOAuth = MANAGED[c.id];
});

// ── Source catalog badge: show "OAuth" tag on managed connectors ─────────────
export function hasManagedOAuth(connectorId: string): boolean {
